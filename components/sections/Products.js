        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PRODUCTS.map((p, i) => (
            <motion.div key={i} variants={fadeUp}
              className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-[var(--color-border)] hover:shadow-[0_24px_64px_rgba(0,0,0,0.10)] transition-all duration-500 hover:-translate-y-2"
            >
              {/* Header */}
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <div className="mt-6 pt-6 border-t border-[var(--color-border)]">
                  <span className="text-[0.875rem] font-semibold text-[var(--color-primary)] group-hover:gap-3 flex items-center gap-2 transition-all duration-200">
                    Learn more
                    <motion.span className="inline-block" whileHover={{ x: 4 }}
                      transition={{ duration: 0.2, ease: [0.34, 1.56, 0.64, 1] }}>→</motion.span>
                  </span>
                </div>
              </div>
            </motion.div>
